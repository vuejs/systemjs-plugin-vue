...
module.exports = function (id, style, name, minify, opts) {
  ...
  if (val) {
    ...
    var compilerPath = opts.compilers && lang && opts.compilers[lang]
    if (compilerPath) {
      return System.import(compilerPath)
        .then(compiler => compiler(css, name))
        .then(applyPostCSS)
    } else {
      return applyPostCSS(css)
    }
    ...
    function applyPostCSS (css) {
      ...
      var optionsPromises = []
      var pos = assign({}, postcssOpts.options)
      if (pos) {
